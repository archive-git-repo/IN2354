...
public:
	...
	void extract_features() {
		...
		for (int i = 0; i < N_FRAMES; i++) {
			orbDetector->detect(this->rgb[i], keypoints.at(i));
		}
		// <-
	...
	void describe_features() {
		...
		cv::Ptr<cv::ORB> orbExtractor = cv::ORB::create();

		for (int i = 0; i < N_FRAMES; i++) {
			orbExtractor->compute(this->rgb[i], keypoints.at(i), descriptors[i]);
		}
		// <-
	}
	...
	void brute_force(int idx0, int idx1, std::vector<std::pair<int, int>> &matches) {
		...
		for (int i = 0; i < descriptors[idx0].rows; i++) {
			...
			for (int j = 0; j < descriptors[idx1].rows; j++) {
				...
				int hammingDistance = hamming_distance(d0, d1);
				
				if (hammingDistance < matches[i].second) {
					matches[i].second = hammingDistance;
					matches[i].first = j;
				}
			}
		}
	...
	void filter_matches(int idx0, int idx1, std::vector<std::pair<int, int>> &matches, std::vector<cv::DMatch> &filtered) {
		...
		int hammingDistanceLimit = 40;
		int distanceLimit = 40;

		for (int i = 0; i < (int)matches.size(); i++) {
			if (matches[i].second < hammingDistanceLimit) {

				cv::KeyPoint keyPoint1 = this->keypoints[idx0][i];
				cv::KeyPoint keyPoint2 = this->keypoints[idx1][matches[i].first];

				double dist_p2p = cv::norm(keyPoint1.pt - keyPoint2.pt);
				if (dist_p2p < distanceLimit) {
					cv::DMatch dMatch = cv::DMatch(i, matches[i].first, matches[i].second);
